class Address < ApplicationRecord
	...
	validates :phone_number, {presence: true, format: { with: /\A\d{2,4}-\d{2,4}-\d{4}\z/, message: "must be valid" }, on:[:create, :update]}
	validates :post_code, {presence: true, format: { with: /\A\d{3}\-?\d{4}\z/, message: "must be valid" }, on:[:create, :update]}
